...
*/
void processer_non_preemptive_init(workload_tt workload, array_tt cores, array_tt threads, int *g_i)
{
    /* Sanity check. */
...
void processer_non_preemptive_execute(struct core *c, int *aux)
{
    /* Sanity check. */
    assert(c != NULL);
    ...
    while ( !queue_empty(core_get_tsks(c)) )
    {
        ...
        */
        ...
        int waiting_time = accum_waiting - task_arrivaltime(ts);
        ...
        // printf("Task %d - Amount processed %d - Waiting time %d - Arrival Time %d - Thread ID %d - Global Iterator %d\n", task_workload(ts), amount_processed, task_waiting_time(ts), task_arrivaltime(ts), task_assigthread(ts), *(processdata.g_iterator));

        /* If a task has finished, we add into a different queue, otherwise, we 'recycle' it into thread. */
...
*/
queue_tt processer_non_preemptive_end(void)
{
...
static struct processer _non_preemptive = {
    ...
    processer_non_preemptive_execute,
