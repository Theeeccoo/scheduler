...
} processdata = { 0, NULL, 0, NULL, NULL, NULL, NULL };
...
*/
void processer_rr_preemptive_init(workload_tt workload, array_tt cores, array_tt threads, int *g_i)
{
    /* Sanity check. */
...
void processer_rr_preemptive_execute(struct core *c, int *aux)
{
    /* Sanity check. */
    assert(c != NULL);
    ...
    while ( !queue_empty(core_get_tsks(c)) )
    {
        ...
        */
        ...
        int amount_processed = 0,
            amount_to_process = (task_work_left(ts) > processdata.quantum) ? processdata.quantum : task_work_left(ts);
        ...
        printf("Task %d - Processed: %d - Left %d - Thread %d\n\n", task_workload(ts), amount_processed, task_work_left(ts), task_assigthread(ts));
    }
...
*/
queue_tt processer_rr_preemptive_end(void)
{
...
static struct processer _rr_preemptive = {
    ...
    processer_rr_preemptive_execute,
